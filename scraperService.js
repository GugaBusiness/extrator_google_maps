...
/**
 * Main scraping task runner
 * @param {Object} options Options object
 * @param {string} options.query Google Maps query
 * @param {number} options.limit Max results to fetch
 * @param {boolean} options.headless Headless browser flag
 * @param {string} [options.userId] Supabase User ID (for SaaS credit controls and association)
 * @param {string} [options.searchId] Supabase Search ID (for direct real-time lead sync)
 * @param {Function} [options.onProgress] Progress callback function: (type, data) => {}
 */
async function runScrape({ query, limit = 10, headless = true, userId = null, searchId = null, onProgress = () => {} }) {
  const sendEvent = onProgress;
  ...
  try {
    ...
    sendEvent('status', { message: 'Rolando feed para localizar resultados...' });
    ...
    async function scrapeWorker() {
      while (urlIndex < uniqueUrls.length) {
        ...
        try {
          ...
          // Save progress incrementally locally
          fs.writeFileSync(jsonPath, JSON.stringify(leads, null, 2), 'utf-8');
          fs.writeFileSync(csvPath, convertToCSV(leads), 'utf-8');

          // REAL-TIME SAAS CLOUD SYNC: If searchId is provided, save lead directly to Supabase!
          if (searchId) {
            try {
              ...
              if (dbError) {
                console.error(`❌ [SUPABASE SYNC] Erro ao gravar lead "${lead.name}":`, dbError.message);
              }
            } catch (err) {
              console.error(`❌ [SUPABASE SYNC] Falha de conexão ao salvar lead:`, err.message);
            }
          }
